...
{
	tc_set_compare_value(&instance, channel, conf.prescaledTicksPerPeriod * dutyCycle);
}
...
TCMotor0::TCMotor0() : TCMotor(res, TC_COMPARE_CAPTURE_CHANNEL_1)
{
	...
	config.pwm_channel[1].pin_mux = MUX_PA18E_TC3_WO0;
	config.pwm_channel[1].pin_out = PIN_PA18E_TC3_WO0;
 ...
 TCMotor1::TCMotor1() : TCMotor(res, TC_COMPARE_CAPTURE_CHANNEL_1)
{
	tc_config config;
	tc_get_config_defaults(&config);
	config.clock_prescaler = tccPrescaler_to_tcPrescaler(res.prescaleSetting);
	...
	config.oneshot = false;
	config.pwm_channel[1].enabled = true;
	config.pwm_channel[1].pin_mux = MUX_PA22E_TC4_WO0;
	config.pwm_channel[1].pin_out = PIN_PA22E_TC4_WO0;
	config.run_in_standby = true;
	config.wave_generation = TC_WAVE_GENERATION_MATCH_PWM;
	config.waveform_invert_output = 0b1;
