...
void EMC1701::init(TWIManager *const twiManager, uint8_t const address)
{
	...
	FreeRTOSTask::init(config::emc1701::taskName, config::emc1701::taskStackDepth, config::emc1701::taskPriority);
}
...
void EMC1701::addAction(Action const &action)
{
	...
	if(xSemaphoreTake(mtx_regcache, utility::conversions::msToTicks(100)) == pdFALSE) {
...
void emc1701::sensor::EMCSensor::init(EMC1701 *const parent)
{
	this->parent = parent;
}

float emc1701::sensor::Current::voltageDifference()
{
	if(parent == nullptr)
		return 0;
	int16_t adcval;
	*reinterpret_cast<uint16_t *>(&adcval) = (parent->readRegister(emc1701::Register::VsenseHighByte)) << 8 | (parent->readRegister(emc1701::Register::VsenseLowByte));
	return utility::conversions::adc_voltage<int16_t, 0x7fff>(adcval, config::emc1701::referenceVoltage);
}

float emc1701::sensor::Current::value()
{
	return voltageDifference() / config::emc1701::referenceVoltage;
}

float emc1701::sensor::Voltage::value()
{
	if(parent == nullptr)
		return 0;
	uint16_t vsourcevalue = (parent->readRegister(emc1701::Register::VsourceHighByte) << 8) | (parent->readRegister(emc1701::Register::VsourceLowByte));
	//From the emc1701 data sheet
